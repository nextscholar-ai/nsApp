...
def render_student_id_pdf(card: Dict[str, Any], output_pdf_path: str) -> str:
    ...
    try:
        ...
        if logo_path and os.path.exists(logo_path):
            ...
            c.drawImage(img, 20, y - 35, width=35 * mm / 1.0, height=35 * mm / 1.0, mask="auto")
        ...
        y -= 16
        c.drawString(20, y, f"Academic Session: {card.get('academic_session_label','')}")

        # Student photo
        photo_path = card.get("student_photo_path")
        if photo_path and os.path.exists(photo_path):
            c.drawImage(ImageReader(photo_path), width - 90, y - 120, 60, 80, mask="auto")
        ...
        y -= 14
        c.drawString(20, y, f"Valid Till: {valid_till.isoformat() if isinstance(valid_till, date) else ''}")
        y -= 14
