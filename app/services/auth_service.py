...
from app.auth import (
    ...
    verify_refresh_token, verify_access_token, create_reset_token, verify_reset_token, generate_otp
)
...
class AuthService:
    ...
    def authenticate_with_otp(self, email: str, otp: str) -> Optional[User]:
    ...
    def refresh_access_token(self, refresh_token: str) -> Optional[Dict[str, str]]:
        ...
        if not user or not user.is_active or user.is_deleted:
            return None
        return {"access_token": create_access_token({"sub": str(user.id), "role": user.role.value})}
    ...
    def send_reset_token(self, email: str) -> Optional[str]:
        user = self.user_repo.get_by_email(email)
        if not user:
            return None
